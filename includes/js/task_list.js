...
(function($) {
		...
		initSortables : function() {
			...
			api.menuList.sortable({
				...
				stop: function(e, ui) {
					...
					var task_list_menu_order_counter = 0;
					$('#menu-to-edit li').each( function( i, el ) {
						var item = {};
						item.ID = $(el).getItemData()["menu-item-db-id"];
						if ( $(el).getItemData()["menu-item-type"] == 'cp-task-lists' ) {
							task_list_menu_order_counter = 0;
							currentTaskList = $(el);
							item.menu_order = menu_order_counter;
							menu_order_counter++;
						} else {
							if ( $(el).hasClass( 'menu-item-depth-1') === false ) {
								currentTaskList = null;
								item.menu_order = menu_order_counter;
								menu_order_counter++;
							} else {
								item.menu_order = task_list_menu_order_counter;
								task_list_menu_order_counter++;
								item.task_list = currentTaskList.getItemData()["menu-item-db-id"];
							}
						}
						if ( $(el).getItemData()["menu-item-type"] == 'cp-tasks' ) {
	...
	$(document).ready(function(){ cpTaskList.init(); });
